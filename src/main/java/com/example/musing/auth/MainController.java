...
public class MainController {
    ...
    @GetMapping("main")
    public ResponseEntity<?> mainPage(){//비로그인 기준 Dto와 로그인 기준 Dto파일을 다르게 보내기 위해 와일드카드 사용
        ...
        if(Objects.equals(auth.getAuthorities().toString(), "ROLE_USER")){
                String check = userService.checkInputTags(auth.getName());//유저가 분위기 및 장르, 좋아하는 아티스트를 넣었는지 확인하기
            ...
            //음악 추천 게시판 및 공지사항 정보 보내줄 Dto 추가하기(비로그인 기준)
            //작성 미완료단계면 이 부분도 Dto에 넣어서 전송해야함, 프론트에서 상황에 맞게 모달 띄우기 위해
        }
